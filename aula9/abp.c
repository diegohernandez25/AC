...
{ 
	...
	if((*proot)->PtLeft==NULL) NodeDelete(proot);
	else ABPDeleteMin(&(*proot)->PtLeft);
...
{
	PtABPNode Del =*proot, Prev=NULL, Node;
	if(*proot == NULL){
		Error=ABP_EMPTY;
		return;
	}
	while(Del->PtRight !=NULL){
		Prev=Del;
		Del=Del->PtRight;
	}
	Node=Del;
	...
	if(Node->PtRight ==NULL) Node=Node->PtLeft;
	else Node=Node->PtRight;
	
	if(Del==*proot)	*proot=Node;
	else if(Prev->PtLeft==Del) Prev->PtLeft=Node;
	else Prev->PtRight=Node;
...
PtABPNode ABPFloorValue (PtABPNode proot, int pvalue)
{
	...
	Error=OK;
	while(proot!=NULL){
		if(tmp==NULL && ((proot->Elem) < pvalue)) tmp=proot;
		
		if(((proot->Elem) < pvalue) && ((proot->Elem) > (tmp->Elem))) tmp=proot;
		
		if(proot->Elem==pvalue) return proot;
		else if((proot->Elem)>pvalue && (proot->PtLeft)!=NULL) proot=proot->PtLeft;
		else if((proot->Elem)<pvalue && (proot->PtRight)!=NULL) proot=proot->PtRight;
		else break;
	}
	if(tmp==NULL){
		Error=NO_ELEM;
...
PtABPNode ABPCeilValue (PtABPNode proot, int pvalue)
{
	...
	if(proot->Elem==pvalue) return proot;	
	else if((proot->Elem) > pvalue && (proot->PtLeft)!=NULL) tmp=ABPCeilValue(proot->PtLeft, pvalue);
	else if((proot->Elem) < pvalue && (proot->PtRight)!=NULL) tmp=ABPCeilValue(proot->PtRight, pvalue);
	else{
		if((proot->Elem) > pvalue) return proot;
		return NULL;	
	}
	...
	//else if(((tmp->Elem) < (proot->Elem)) && ((proot->Elem) > pvalue)) return tmp;
	else if(((tmp->Elem) > (proot->Elem)) && ((proot->Elem) > pvalue)) return proot;
	else return tmp;
...
{ /* função recursiva - insira o seu codigo */ 
	...
	Error=OK;
	if((proot->Elem) >= pvalue && (proot->PtLeft)!=NULL) count=ABPRank(proot->PtLeft, pvalue);

	else if((proot->Elem) < pvalue){
		count=1;
		if(proot->PtRight!=NULL) count+=ABPRank(proot->PtRight, pvalue);
		if(proot->PtLeft!=NULL) count+=ABPRank(proot->PtLeft, pvalue);
	}
	return count;
}
